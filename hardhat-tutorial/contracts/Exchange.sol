...
contract Exchange is ERC20 {
    ...
    // return the amount of Crypto Dev token hold by contract
    ...
    function addLiquidity(uint _amount) public payable returns (uint) {
        ...
        // if the reserve is empty, take in any value of Crypto Dev token and ETH user suply
        // because there is no ratio now
        if (cryptoDevTokenReserve == 0) {
            // transfer the Crypto Dev tokem from user's account to the contract
            cryptoDevToken.transferFrom(msg.sender, address(this), _amount);
            /* take the ETH balance and mint ETH balance amount of LP token to the user
    ...
    /**
     * @dev Returns the amount Eth or Crypto Dev tokens that would be returned to the user
     * in the swap
     */
    function getAmountOfTokens(
        ...
        uint denominator = inputAmountWithFee + inputReserve * 100;
